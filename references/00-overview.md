...
Payload содержит маску `s` (int): биты категорий + **бит 30 (2^30) = «токен только на чтение»**.
Проверено эмпирически (2026-07-23, проект AutoReport): токен с галкой «только на чтение» имеет
`s & (1<<30) != 0`, запись (`POST /content/v3/media/save`) с ним возвращает 401, чтение работает.
