...
> В спеке `05-orders-dbs` поле `servers` пустое (`[]`). Хост взят из `00-overview.md`
> (`Marketplace → marketplace-api.wildberries.ru`) — пути спеки (`/api/v3/dbs/...`,
> `/api/marketplace/v3/dbs/...`) лежат в тех же префиксах, что и FBS
> (`11-marketplace.md`) и подтверждённое семейство Самовывоз (`06-in-store-pickup`,
> там `servers` указан явно).
