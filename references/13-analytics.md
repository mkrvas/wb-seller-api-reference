...
| Остатки (`/api/analytics/v1/stocks-report/wb-warehouses`) | 1 запр/20 сек, **счётчик по IP, а не по токену** (live 2026-08-08) |
| Платное хранение (async) | ограничено по задачам |
| Отчёт приёмки (async) | ограничено по задачам |

> Из rate-limit-заголовков живьём приходит **только `x-ratelimit-remaining`** —
> `X-Ratelimit-Limit` и `X-Ratelimit-Reset` в ответе отсутствуют (live-проверка
> 2026-08-08). Подробности и лимит по IP — `01-rate-limits-retry.md`.
