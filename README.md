...
**Аудит от 2026-07-21:** перед публикацией все файлы прогнаны через сверку со сторонними источниками (официальный `dev.wildberries.ru` на момент проверки блокировал автоматический доступ). Места с неподтверждённой или противоречивой информацией помечены прямо в тексте блоком `⚠️`. После сверки со снапшотами OpenAPI-спек (`specs/`) пути в `18-chat.md`, `20-documents.md`, `21-calendar.md` **подтверждены** (непроверенными остаются лишь отдельные лимиты и поля тел запросов — помечено на месте). Под сомнением остаётся `19-returns.md`: актуальная модель возвратов — это заявки `claims` на хосте `feedbacks-api` (см. `16-feedbacks.md`), а описанные в `19-returns.md` пути `returns-api` официально не подтверждены.
...
**Важно:** если на GitHub висит несмерженный PR от бота (WB что-то поменял, а изменения ещё не влиты в `master`), то данные в установленной у вас копии отстают от актуальных спек. Смержите этот PR и обновите копию.
